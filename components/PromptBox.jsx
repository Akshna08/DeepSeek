...
const PromptBox = ({ setIsLoding, isLoding }) => {
  ...
  const sendPrompt = async (e) => {
    ...
    try {
      ...
      if (isLoding) return toast.error('Please wait for the previous message to finish processing');
      ...
      setChats((prevChat) => prevChat.map((chat) => chat._id === selectedChat._id ? { ...chat, messages: [...chat.messages, userPrompt] } : chat))

      // setting selected chat to the latest chat
      setSelectedChat((prev) => ({
        ...prev, messages: [...prev.messages, userPrompt]
      }))

      const { data } = await axios.post('/api/chat/ai', { chatId: selectedChat._id, prompt })

      if (data.success) {
        setChats((prevChat) => prevChat.map((chat) => chat._id === selectedChat._id ? { ...chat, messages: [...chat.messages, data.data] } : chat))
